...
class Installer(object):
    def __init__(self, django_settings_module):
        ...
        self.req_path = os.path.join(self.project_root, 'reqs', 'prod.txt')
        self.kwargs = {
        ...
        if not os.path.exists(self.req_path):
            raise RuntimeError('reqs/prod.txt 를 생성해주세요.')
    ...
    def run(self, service_only="restart"):
...
wsgi-file = {{ project_root }}/{{ project_name }}/wsgi.py
